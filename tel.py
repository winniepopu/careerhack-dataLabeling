    ...
    def bbox(self, qp):
        ...
        for line in json_line:
            ...
            for word in line['words']:
                ...
                if run_first == False:
                    if lineNum > 2 and lineNum <=5:
                        if wordNum == 0 or  wordNum == 1:
                            if len(word['text'])>=3:
                                if bool(re.search(r'\d', word['text'])):
                        # delset = string.punctuation
                        # deled_str = word['text'].translate(delset)
                        # print("deled_str: ",deled_str)
                        # print(deled_str.isdigit()) 
                        # if deled_str.isdigit():
